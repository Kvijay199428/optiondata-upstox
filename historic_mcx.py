...
def configDB(filename="api/ini/MCX.ini", section="postgresql"):
    parser = ConfigParser()
    ...
    try:
        insert_query = sql.SQL('''
            INSERT INTO {} (timestamp, open_price, high_price, low_price, close_price, volume, open_interest, 
                            strike_price, expiry, lot_size, instrument_type, tick_size, underlying_key, 
                            trading_symbol, freeze_quantity, instrument_key, table_name)
            VALUES %s
...
def main():
    ...
    criteria = {"asset_symbol": "CRUDEOIL"}
    filtered_instruments = [instr for instr in instruments_data 
                            if instr.get("asset_symbol") == criteria["asset_symbol"]]
