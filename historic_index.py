...
log_file = "api/logs/historic_index.log"
...
# Create table if not exists with dual indexing
def create_table(conn, table_name, instrument_key):
    logging.debug(f"Creating table {table_name} if not exists with indexing.")
    cur = conn.cursor()
...
def main():
    ...
    file_choice = input("Enter the number corresponding to your choice select 'complete.json': ")
    try:
