...
log_file = "api/logs/OC_Sensex.log"
...
def get_next_friday(year, month, today=None):
    ...
    if days_ahead == 0:  # If today is Friday, move to next week
        days_ahead = 7
    ...
    # If the next Friday is a holiday or a weekend, get the previous trading day
    while next_friday.weekday() > 4 or next_friday in india_holidays:
        logging.info(f"{next_friday.date()} is a weekend or holiday. Finding the previous trading day.")
        next_friday -= timedelta(days=1)

    next_friday_date = next_friday.date()  # Extract the final date
    logging.info(f"Next Friday expiry date is {next_friday_date}.")
    
    return next_friday_date  # Add this return statement
...
class OptionChainFetcher:
    def fetch_data(self):
        ...
        if expiry_date is None:
            logging.error("Failed to get expiry date")
            return {}

        params = {
            'instrument_key': 'BSE_INDEX|SENSEX',
            'expiry_date': str(expiry_date)  # Convert date to string
        }
        headers = {
