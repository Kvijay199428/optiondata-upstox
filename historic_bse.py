...
log_file = "api/logs/historic_bse.log"
...
def configDB(filename="api/ini/BSE.ini", section="postgresql"):
    parser = ConfigParser()
...
def create_table(conn, table_name, instrument_key):
    ...
    try:
        ...
        # Create fifth index based on name
        cur.execute(sql.SQL("""
            CREATE INDEX IF NOT EXISTS {} 
            ON {} (name)
        """).format(
        ...
        # Create sixth index based on short_name
        cur.execute(sql.SQL("""
            CREATE INDEX IF NOT EXISTS {} 
            ON {} (short_name)
        """).format(
            sql.Identifier(f"idx_{table_name}_short_name"),
            sql.Identifier(table_name)
        ))
    ...
    try:
        insert_query = sql.SQL('''
            INSERT INTO {} (timestamp, open_price, high_price, low_price, close_price, volume, open_interest, 
                            instrument_type, name, short_name, trading_symbol, instrument_key, table_name)
            VALUES %s
            ON CONFLICT (timestamp) DO NOTHING;
        ''').format(sql.Identifier(table_name))

        # Handle the missing 'short_name' by assigning a default value
        short_name = instrument.get('short_name', 'N/A')
        ...
        extras.execute_values(
            ...
            [(row[0], row[1], row[2], row[3], row[4], row[5], row[6], 
              ...
              instrument['name'],
              short_name,  # Use the value from the get() method
...
def main():
    ...
    file_choice = input("Enter the number corresponding to your choice select 'bse.json': ")
    try:
...
#         insert_query = sql.SQL('''
#             INSERT INTO {} (timestamp, open_price, high_price, low_price, close_price, volume, open_interest, 
#                             instrument_type, name, short_name
