...
filename = "api/token/accessToken_order.txt"
...
# Function to filter DataFrame based on lot size
...
# Function to find the closest options based on premium range
...
# Function to fetch quotes for the instruments
...
# WebSocket market data feed authorization
...
# Filter DataFrame and find options
...
while True:
    sleep(1)
    for name in symbol:
        # Ensure data_dict has the required structure before accessing
        if name in data_dict.get('feeds', {}) and 'ltpc' in data_dict['feeds'][name]:
            ltp = data_dict['feeds'][name]['ltpc']['ltp']
            print(name, ltp)

            if ltp >= (trade_symbol[name] * buy_percent) / 100 and trade is None:
            ...
            print(f"Data not available for {name}. Waiting for updates.")
