...
def configDB(db_ini="api/ini/test.ini", section="postgresql"):
    parser = ConfigParser()
    parser.read(db_ini)
    db = {}
    if parser.has_section(section):
        ...
        logging.error(f"Section {section} not found in {db_ini} file.")
        raise Exception(f'Section {section} not found in {db_ini} file.')
    return db
